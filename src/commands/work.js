...
const fishing   = require('./work/fishing');
const mining    = require('./work/mining');
const streaming = require('./work/streaming');
...
function getCorporateByTier(t) { return CORPORATE_JOBS.find(j => j.tier === t)  || null; }
function getActivityJob(id)    { return ACTIVITY_JOBS.find(j => j.id === id)    || null; }
function getAllJobs()           { return [...ACTIVITY_JOBS, ...CORPORATE_JOBS]; }
    ...
    async execute(interaction) {
        ...
        if (sub === 'work') {
            ...
            if (user.lastWork && now - user.lastWork < CORPORATE_COOLDOWN) {
                const left = CORPORATE_COOLDOWN - (now - user.lastWork);
        ...
        if (sub === 'apply') {
            ...
            if (totalWealth < job.requiresBalance) {
                return interaction.reply({
                    embeds: [new EmbedBuilder()
                        .setTitle('Application Denied')
                        .setDescription(`**${job.title}** requires **$${formatNumber(job.requiresBalance)}** total wealth.\nYou have **$${formatNumber(totalWealth)}**.`)
                        .setColor(0xff0000)],
                    ephemeral: true,
                });
            }

            if (prestige < (job.requiresPrestige ?? 0)) {
                return interaction.reply({
                    embeds: [new EmbedBuilder()
                        .setTitle('Application Denied')
            ...
            const isActivity = !!getActivityJob(jobId);
            return interaction.reply({
                embeds: [new EmbedBuilder()
